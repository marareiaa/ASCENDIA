...
- Servidor web local (opcional, mas recomendado para evitar restrições de CORS com `sessionStorage`).
...
Basta abrir o arquivo `index.html` diretamente no navegador. Algumas funcionalidades que dependem de `sessionStorage` podem ter comportamento limitado nesse modo.

### Hospedagem

O projeto é compatível com qualquer serviço de hospedagem estática, incluindo GitHub Pages, Netlify e Vercel. Por ser puramente estático (HTML + CSS + JS), não requer configuração de servidor ou banco de dados.

Passo a passo para publicação no GitHub Pages:

1. Suba todo o conteúdo do projeto para a branch principal do repositório, mantendo a estrutura de pastas descrita acima.
2. Acesse Settings → Pages no repositório.
3. Em Source, selecione a branch `main` e a pasta raiz (`/`).
4. Clique em Save e aguarde alguns minutos até a publicação.
5. O GitHub vai gerar uma URL no formato `https://seu-usuario.github.io/nome-do-repositorio/`, que deve ser preenchida na seção [Demonstração](#demonstração) deste README.
...
- [W3C WAI-ARIA Accessibility Guidelines](https://www.w3.org/WAI/standards-guidelines/aria)
...
| | |
|---|---|
| Projeto | Ascendia — Plataforma de Otimização de Currículos |
| Tipo de entrega | Trabalho Final — Programação Front-End |
| Instituição | UniCesumar |
| Disciplina | Programação Front-End |
| Versão | 1.0.0 |
| Status | Entrega final |
| Protótipo de design | [Canva](https://canva.link/fhbe6o1m9m03bdm) |
